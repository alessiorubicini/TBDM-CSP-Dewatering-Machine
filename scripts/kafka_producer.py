...
def simulate_sensor_data():
	while True:
		...
		timestamp = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(sensor_data["timestamp"] / 1e9))
		print(f"{timestamp}: Sent to Kafka [{message_value}]")  # Print the sent message (for debugging)
